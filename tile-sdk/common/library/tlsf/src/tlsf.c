...
{
#if 0 // for debug -- ertl-liyixiao
	printf(">> split block bytes=%lu", block_size(block));
#endif
...
#if 0 // for debug -- ertl-liyixiao
	printf(">>  into %lu and %lu", block_size(block), block_size(remaining));
#endif
...
#if 0 // for debug -- ertl-liyixiao
	printf(">> add pool bytes=%lu", pool_bytes);
	printf(">> first blk bytes=%lu", block_size(block));
	control_t* control = tlsf_cast(control_t*, tlsf);
	for (int i = 0; i < FL_INDEX_COUNT * SL_INDEX_COUNT; ++i)
		if (control->blocks[0][i]->size != 0) printf(">> blocksz=%lu", block_size((control->blocks[0][i])));
#endif
...
void* tlsf_malloc(tlsf_t tlsf, size_t size)
{
	...
	if (!p) {
		printf("%s() tlsf=%p, adjust=%lu, block=%p", __FUNCTION__, tlsf, adjust, block);
		for (int i = 0; i < FL_INDEX_COUNT * SL_INDEX_COUNT; ++i)
			if (control->blocks[0][i]->size != 0) printf(">> blocksz=%lu", block_size((control->blocks[0][i])));
	}
	return p;
...
void tlsf_free(tlsf_t tlsf, void* ptr)
{
	...
	if (ptr)
	{
		...
		block_header_t* block = block_from_ptr(ptr);
#if 0 // for debug -- ertl-liyixiao
	printf(">> free block bytes=%lu", block_size(block));
#endif
...
#if 0 // for debug -- ertl-liyixiao
	printf(">> new block bytes=%lu after free", block_size(block));
#endif
		block_insert(control, block);
	}
}
