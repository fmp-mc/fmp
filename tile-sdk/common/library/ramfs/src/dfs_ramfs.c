...
#include <kernel.h>
#include <t_syslog.h>
#define rt_memheap_alloc(memheap,size) malloc((size))
#define rt_memheap_realloc(memheap,ptr,size) realloc((ptr),(size))
#define rt_memheap_free free
#define rt_strncpy strncpy
#define rt_strcmp strcmp
#define rt_set_errno(no) assert(0) // TODO: check this
#define rt_memheap_init(...) (RT_EOK) // TODO: check this
#define rt_object_detach(...)
...
#define FILE_MAX_SIZE_INCREMENT (100 * 1024 * 1024ULL)
...
#if defined(FILE_MAX_SIZE_INCREMENT)
    if (count + fd->pos > fd->max_size)
    {
    	rt_size_t incr = count + fd->pos - fd->max_size;
    	if (incr < FILE_MAX_SIZE_INCREMENT) incr = FILE_MAX_SIZE_INCREMENT;
        rt_uint8_t *ptr;
        ptr = rt_memheap_realloc(&(ramfs->memheap), dirent->data, fd->max_size + incr);
        if (ptr == RT_NULL)
        {
