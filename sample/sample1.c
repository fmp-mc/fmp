...
{
	// TODO: debug code -- ertl-liyixiao
//	syslog(LOG_EMERG, "Server task %d starts.", (int_t) exinf);
//	syslog(LOG_EMERG, "interrupt context %d.", (int_t) sense_context());

	raise_int();
    //target_ipi_raise(1);
	//ER ercd = act_tsk(SERVER_TASK2);rot_rdq(SERVER_PRIORITY)
	//syslog(LOG_EMERG, "ercd %d.", ercd);
    return;
	while(1) {
		syslog(LOG_EMERG, "Server task %d running.", (int_t) exinf);
		rot_rdq(SERVER_PRIORITY);
	}
...
void task(intptr_t exinf)
{
	...
	SVC_PERROR(ena_tex());
	while (1) {
		syslog(/*!x_sense_mprc() ? LOG_EMERG : */LOG_NOTICE, "task%d_%d is running (%03d).   %s",
			   classno, tskno, ++n, graph[tskno-1]); if (n >= 50) if (!x_sense_mprc()) debug_loop();
...
void main_task(intptr_t exinf)
{
	...
	do {
		...
		//syslog(LOG_EMERG, "HERE2! ret");
		SVC_PERROR(serial_rea_dat(serial_port[exinf-1], &c, 1));
		//syslog(LOG_EMERG, "HERE3! ret");
		switch (c) {
		case 'p':
		case 'e':
		case 's':
