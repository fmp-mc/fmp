...
namespace toppers
{
   ...
   */
  //int s_record::operator[]( size_type address ) const // Why does address use size_type ? -- ertl-liyixiao
  int s_record::operator[]( symbol_addr_type address ) const
  {
#if 1 // Debug for 'TOPPERS_cfg_magic_number' -- ertl-liyixiao
for (std::vector< value_type >::const_iterator it=data_.begin(); it!=data_.end(); ++it)
    {
        std::cout << "Rec addr: " <<  std::hex << it->first << '\n';
