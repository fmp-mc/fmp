...
#include <sstream>
#include <cctype>
#include <iostream> // ertl-liyixiao
...
namespace toppers
{
  ...
  void nm_symbol::load( std::istream& istr )
  {
    while ( istr )
    {
      ...
      isstr >> type >> symbol;
      std::cout << "Read symbol " << symbol << std::endl; // -- ertl-liyixiao
...
#if 1 // Debug for 'TOPPERS_cfg_magic_number' -- ertl-liyixiao
      if (symbol == "TOPPERS_cfg_magic_number") std::cout << "Try to find symbol " << symbol << std::endl; 
for (std::map<std::string, entry>::const_iterator it=symbol_map_.begin(); it!=symbol_map_.end(); ++it)
    if (symbol == it->first) {
        std::cout << "Has symbol: " << iter->first << '\n';
        std::cout << "Has addr: " <<  std::hex << e.address << '\n';
        std::cout << "Has type: " <<  e.type << '\n';
    }
#endif
    return e;
  }
