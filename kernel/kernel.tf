...
$IF LENGTH(SUPPORT_MANY_CORE_AFFINITY)$
	$SPC$($ALT(TSK.TEXATR[tskid],"TA_NULL")$), ($ALT(TSK.TEXRTN[tskid],"NULL")$), $CLASS_AFFINITY_INI[TSK.CLASS[tskid]]$, $AFFINITY_MASK_TO_C_CODE(CLASS_AFFINITY_MASK[TSK.CLASS[tskid]])$ }
$ELSE$
...
$IF LENGTH(SUPPORT_MANY_CORE_AFFINITY)$
			$TAB${ ($CYC.CYCATR[cycid]$), (intptr_t)($CYC.EXINF[cycid]$), ($CYC.CYCHDR[cycid]$), ($CYC.CYCTIM[cycid]$), ($CYC.CYCPHS[cycid]$), $CLASS_AFFINITY_INI[CYC.CLASS[cycid]]$, $AFFINITY_MASK_TO_C_CODE(CLASS_AFFINITY_MASK[CYC.CLASS[cycid]])$ }
$ELSE$
...
$FOREACH inhno INH.ORDER_LIST$
$IF LENGTH(SUPPORT_MANY_CORE_AFFINITY)$
    $INH.AFFINITY_MASK[inhno] = GET_AFFINITY_INI(CLASS_AFFINITY_INI[INH.CLASS[inhno]])$
$ELSE$
...
$IF LENGTH(INH.ORDER_LIST)$
	...
	$JOINEACH inhno INH.ORDER_LIST ",\n"$
		$IF (INH.INHATR[inhno] & TA_NONKERNEL) == 0$
$IF LENGTH(SUPPORT_MANY_CORE_AFFINITY)$
			$TAB${ ($inhno$), ($INH.INHATR[inhno]$), (FP)(INT_ENTRY($inhno$, $INH.INTHDR[inhno]$)), $CLASS_AFFINITY_INI[INH.CLASS[inhno]]$, $AFFINITY_MASK_TO_C_CODE(INH.AFFINITY_MASK[inhno])$ }
$ELSE$
...
$IF LENGTH(SUPPORT_MANY_CORE_AFFINITY)$
		$TAB${ ($intno$), ($INT.INTATR[intno]$), ($INT.INTPRI[intno]$),  $CLASS_AFFINITY_INI[INT.CLASS[intno]]$, $AFFINITY_MASK_TO_C_CODE(INT.AFFINITY_MASK[intno])$}
$ELSE$
