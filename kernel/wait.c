...
//        syslog(LOG_NOTICE, "%s() unlock %d @ prcid %d", __FUNCTION__, TSKID(((TCB*)p_wait_queue->p_next)), p_tcb->p_pcb->prcid);
#if 0
        syslog(LOG_NOTICE, "%s() will unlock: ", __FUNCTION__);
#endif
		p_tcb = (TCB *) queue_delete_next(p_wait_queue);
        //syslog(LOG_NOTICE, "%s() unlock %d @ prcid ", __FUNCTION__, TSKID(p_tcb));
		/* タスクロックの取得 */
		...
		}// else if (p_pcb != get_my_p_pcb()) dispatch_request(p_pcb); // workaround
#if 0
        syslog(LOG_NOTICE, "%s() unlock %d (stat=%d)@ prcid %d, primap:0x%x", __FUNCTION__, TSKID(p_tcb), p_tcb->tstat, p_tcb->p_pcb->prcid, p_tcb->p_pcb->ready_primap);
#endif
		release_nested_tsk_lock(p_pcb);//release_tsk_lock(p_pcb); TODO: !!!!!!!!!!!!!!!!!!!!!!!!!! WRONG IN GIANT LOCK MODE !!!!!!!!!!!!!!!!!!!!!!!! -- ertl-liyixiao
	}
